          ...
          {sorted.map((entry) => {
            ...
            return (
              ...
              >
                <span
                  aria-hidden="true"
                  className="bg-ink-100 text-ink-400 ring-ink-50 border-ink-200 dark:bg-ink-800 dark:ring-ink-950 dark:border-ink-800 absolute top-0 -left-[13px] grid h-6 w-6 place-items-center rounded-full border ring-4"
                >
                ...
                <h3 className="text-ink-950 dark:text-ink-50 mt-1.5 text-lg leading-snug font-semibold text-pretty sm:text-xl">
                  {entry.degree}
                  <span className="text-ink-600 dark:text-ink-400 font-normal"> &middot; {entry.institution}</span>
                </h3>

                {entry.field ? (
                  <p className="text-ink-600 dark:text-ink-400 mt-1 text-pretty">{entry.field}</p>
                ) : null}
